...
| 12 | [12-AGENT-MANAGER-PHASE-3.md](12-AGENT-MANAGER-PHASE-3.md) | Phase-3 work after Manus shipped Phase-2. Reconciles workstream 09 with `C:\Projects\accuracy\MANUS_AGENT_BUILDER_REQUIREMENTS.md` (which adds the knowledge-grounding model: facts + guardrails + authority + reference docs). Calls out path-rename concern. | Active |
| 13 | [13-MANUS-NEXT-ACTIONS.md](13-MANUS-NEXT-ACTIONS.md) | **Self-contained Manus deliverable.** Five sequenced tasks (path check, Knowledge Sources panel, dry-run test, search/filter, versioning) with full GL access details, sample queries, ASCII mockups, acceptance criteria. Pasteable into Manus without reading anything else. | **Ready for handoff** |
